...
| Runs every minute, all **cancelled**, none executing | **GitHub is not allocating runners.** Nothing to fix here — see below | Wait. Confirm with the job API: `runner_id: 0` and no `started_at` progress means it never got a machine |
