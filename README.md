...
>
> **The callback path is therefore not covered by this switch, by design.** Gating the ping on both jobs would mean a Sheets hiccup raising a "the watcher is dead" alert while job alerting is perfectly healthy — a false alarm on the highest-severity channel for a much lower-severity fault, which is how you teach yourself to ignore it. The tradeoff is that a persistently failing `process_applies` (revoked service account, deleted spreadsheet) is silent apart from red runs in the Actions tab: taps stop reaching the Sheet and buttons stop being ticked, while alerts keep arriving normally. A transient failure re-reads the tap on the next run rather than losing it, since `last_update_id` only advances once the job commits — but **the retry is not clean.** Within one tap the order is `append_row` → `answerCallbackQuery` → `editMessageText` → offset. A failure after the append but before the commit lands leaves the row in the Sheet while `pending` still holds the hash, so the retry appends it a second time. Expect duplicate Sheet rows after a red `process_applies` **that got as far as appending**, and check the Sheet rather than assuming the retry cleaned up after itself. Closing both this and the persistent case needs its own signal — see `FUTURE_IMPROVEMENTS.md`.
>
> **A failed *poll* is the one red run that appends nothing.** Since 2026-08-03 the job no longer dies when `getUpdates` is unreachable: it counts the failure, logs `Could not poll Telegram (…); N consecutive failure(s)`, and exits green. Only at ten consecutive failures — about ten minutes — does it go red, with `Telegram polling has failed N runs in a row`. Nothing was appended or edited on any of them, so there is no duplicate row to look for; the taps are still queued at Telegram, which keeps them for 24 hours from when they were made. Anything the poll cannot fix on its own is still immediate: a revoked `TELEGRAM_BOT_TOKEN` is a 401 and fails the run on the spot. The flip side is that a **green** `process_applies` no longer proves taps were read — up to nine consecutive polls can have failed quietly, so read the step log, not just the tick.
...
`process_applies` also clears one itself: a 409 is exactly how Telegram announces an active webhook, so the job treats it as the cue to call `getWebhookInfo`, delete the webhook and retry the poll once. It used to ask `getWebhookInfo` on every run instead — 1,440 calls a day to confirm that a webhook nobody set was still unset, each one an unguarded way for a slow Telegram to fail the job. If the deletion does not take, the run counts it as a failed poll rather than going red on the spot.

## Tests

Pure parsing and dedup logic lives in `watcher/core.py`; the workflow imports it and keeps only its I/O. `tests/conftest.py` extracts the python block straight out of `watch-files.yml` and runs it against a temporary checkout with `urlopen` stubbed, so the end-to-end tests drive the same code that runs in CI rather than a reimplementation. Both blocks are covered that way: `tests/test_workflow.py` drives the watcher, `tests/test_callbacks.py` the `✅ Applied` job, with `google-auth` faked into `sys.modules` so CI still needs nothing but `pytest` and `pyyaml`.
